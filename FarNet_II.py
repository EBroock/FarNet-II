...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_channel, num_filter, b_h_w, kernel_size=3, stride=1, padding=1,device='cuda'):
        super().__init__()
        self.device = device
    ...
    def forward(self, inputs=None, states=None, seq_len=11,device='cuda'):
...
class UNet(nn.Module):
    ...
    def __init__(self, n_channels=1, n_classes=1, n_seq=11, n_hidden=16,device='cuda',batch=30):
        ...
        self.LSTM1 = ConvLSTM(input_channel=8*n_hidden, num_filter=8*n_hidden, b_h_w=(batch,9,11),device=device)
        self.LSTM1_inv = ConvLSTM(input_channel=8*n_hidden, num_filter=8*n_hidden, b_h_w=(batch,9,11),device=device)
        ...
        self.LSTM2 = ConvLSTM(input_channel=4*n_hidden, num_filter=4*n_hidden, b_h_w=(batch,18,22),device=device)
        self.LSTM2_inv = ConvLSTM(input_channel=4*n_hidden, num_filter=4*n_hidden, b_h_w=(batch,18,22),device=device)
        ...
        self.LSTM3 = ConvLSTM(input_channel=2*n_hidden, num_filter=2*n_hidden, b_h_w=(batch,36,45),device=device)
        self.LSTM3_inv = ConvLSTM(input_channel=2*n_hidden, num_filter=2*n_hidden, b_h_w=(batch,36,45),device=device)
        self.conv3 = inconv(n_seq*2,n_seq)
        self.att3 = AttentionBlock(2*n_hidden,2*n_hidden,n_hidden)
        self.up3 = up(4*n_hidden,n_hidden)
        self.LSTM4 = ConvLSTM(input_channel=n_hidden, num_filter=n_hidden, b_h_w=(batch,72,90),device=device)
        self.LSTM4_inv = ConvLSTM(input_channel=n_hidden, num_filter=n_hidden, b_h_w=(batch,72,90),device=device)
